...
export class InternalSecurityGuard implements CanActivate {
  private readonly logger = new Logger('InternalSecurityGuard'); // Contexto del Log
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const expectedApiKey = this.configService.get<string>('AUTH_INTERNAL_API_KEY');
    ...
    // 2. Validar JWT
    try {
      const gatewayPublicKey = this.configService.get<string>('GATEWAY_PUBLIC_KEY_BASE64');
