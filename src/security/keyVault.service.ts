...
export class KeyVaultService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      await this.cacheKey('AUTH_PRIVATE_KEY_BASE64', 'private');
  ...
  async unpack(envelope: string) {
    try {
      const myPrivKey = this.keyCache.get('AUTH_PRIVATE_KEY_BASE64');
      ...
      this.logger.error(`Error de seguridad: ${error.message}`);
